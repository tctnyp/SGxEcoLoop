...
type ReviewSubmission = {
  ...
  aiLabel: string | null;
  aiAccepted: boolean;
  aiDetections: Array<{ label: string; confidence: number }>;
...
function Reviews({ token, showToast }: { token: string; showToast: (message: string) => void }) {
  ...
  const load = () => fetch('/api/portal/submissions?scope=all', { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.json()).then((result) => setSubmissions(result.submissions ?? []));
  useEffect(() => { load().catch(() => undefined); }, [token]);
  const visibleSubmissions = submissions.filter((submission) => scope === 'all' || submission.status === 'pending');
  const item = visibleSubmissions[active];
  ...
  if (!item) return <section className="surface empty-review"><Icon name="check" size={34}/><h2>{scope === 'pending' ? 'Review queue is clear' : 'No YOLO results yet'}</h2><p>{scope === 'pending' ? 'There are no submissions waiting for staff.' : 'Results will appear after a member submits photo evidence.'}</p>{scope === 'pending' && submissions.length > 0 ? <button className="soft-button" onClick={() => { setScope('all'); setActive(0); }}>View all YOLO results</button> : null}</section>;
  const pendingCount = submissions.filter((submission) => submission.status === 'pending').length;
  return <div className="review-layout"><section className="review-queue surface"><div className="section-heading"><div><p className="eyebrow">{pendingCount} NEED REVIEW</p><h3>YOLO results</h3></div></div><div className="review-filters"><button className={scope === 'all' ? 'active' : ''} onClick={() => { setScope('all'); setActive(0); }}>All</button><button className={scope === 'pending' ? 'active' : ''} onClick={() => { setScope('pending'); setActive(0); }}>Needs review</button></div><div className="submission-list">{visibleSubmissions.map((submission, index) => <button key={submission.id} className={active === index ? 'submission active' : 'submission'} onClick={() => { setActive(index); setAssisted(false); }}><span className={`avatar tone-${index % 3}`}>{submission.task[0]}</span><span><b>{submission.task}</b><small>{submission.status === 'approved' ? 'YOLO approved' : submission.status.replace('_', ' ')}</small><em>{new Date(submission.createdAt).toLocaleString()}</em></span><Icon name="arrow"/></button>)}</div></section><section className="review-detail surface"><div className="review-title"><span className="avatar large">{item.task[0]}</span><div><p className="eyebrow">PHOTO EVIDENCE</p><h2>{item.task}</h2><p>{new Date(item.createdAt).toLocaleString()}</p></div><span className={`status ${item.status === 'approved' ? 'open' : 'draft'}`}>{item.status === 'approved' ? 'verified' : item.status.replace('_', ' ')}</span></div><div className="evidence-grid single"><img src={item.photoDataUrl} alt="Member task evidence" className="evidence-photo"/></div><div className="submission-note"><p className="eyebrow">MEMBER DESCRIPTION</p><p>{item.note}</p></div><div className={`ai-card ${assisted ? 'expanded' : ''}`}><span className="ai-icon"><Icon name="spark"/></span><div className="ai-card-copy"><p className="eyebrow">YOLO CHECK</p><b>{item.aiAccepted ? 'Automatically verified' : item.aiConfidence === null ? 'Manual review required' : `${Math.round(item.aiConfidence * 100)}% · ${item.aiLabel ?? 'task match'}`}</b><p>{item.aiDecisionReason ?? (item.aiConfidence === null ? 'YOLO could not safely verify this task.' : 'Staff makes the final decision below the 80% threshold.')}</p></div><button className="soft-button" onClick={() => setAssisted((current) => !current)}>{assisted ? 'Hide results' : 'View results'}</button>{assisted ? <div className="ai-results"><div className="ai-result-meta"><span><small>MODEL</small><b>{item.aiModel ?? 'Unavailable'}</b></span><span><small>PROCESSING</small><b>{item.aiProcessingMs === null ? '—' : `${item.aiProcessingMs.toFixed(1)} ms`}</b></span><span><small>VERDICT</small><b>{item.aiAccepted ? 'Accepted' : 'Staff review'}</b></span></div><p className="ai-summary">{item.aiSummary ?? 'No detection summary was returned for this submission.'}</p><div className="detection-list">{item.aiDetections.length ? item.aiDetections.map((detection, index) => <div key={`${detection.label}-${index}`}><span>{detection.label}</span><div><i style={{ width: `${Math.round(detection.confidence * 100)}%` }}/></div><b>{(detection.confidence * 100).toFixed(1)}%</b></div>) : <p>No task-relevant objects were recorded.</p>}</div></div> : null}</div>{item.status === 'pending' ? <><div className="reward-row"><div><p className="eyebrow">REWARD</p><h3>Assign leaves</h3></div><div className="points-stepper"><button onClick={() => setPoints(Math.max(0, points - 10))}>−</button><label><input type="number" value={points} onChange={(event) => setPoints(Number(event.target.value))}/><small>leaves</small></label><button onClick={() => setPoints(points + 10)}>+</button></div></div><div className="decision-actions review-actions"><button className="danger-button" onClick={() => void remove()}>Delete</button><button className="soft-button" onClick={() => void decide('changes_requested')}>Request changes</button><button className="primary" onClick={() => void decide('approved')}><Icon name="check"/>Approve · {points}</button></div></> : <div className="verified-reward"><span><Icon name="check"/></span><div><p className="eyebrow">AUTOMATED REWARD</p><b>{item.points ?? 0} leaves awarded</b></div></div>}</section></div>;
}
